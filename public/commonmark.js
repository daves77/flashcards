...
(function (global, factory) {
    ...
    var lib = createCommonjsModule(function (module, exports) {
        ...
        function decode(data, level) {
            return (
                !level || level <= 0 ? decode$2.decodeXML : decode$2.decodeHTML
            )(data);
        }
        ...
        function decodeStrict(data, level) {
            return (
                !level || level <= 0
                    ? decode$2.decodeXML
                    : decode$2.decodeHTMLStrict
            )(data);
        }
        exports.decodeStrict = decodeStrict;
        function encode(data, level) {
            return (
                !level || level <= 0 ? encode$1.encodeXML : encode$1.encodeHTML
            )(data);
        }
    ...
    var handleDelim = function (cc, block) {
        ...
        if (
            ...
            (this.options.smart ||
                (cc !== C_SINGLEQUOTE && cc !== C_DOUBLEQUOTE))
        ) {
            this.delimiters = {
                cc: cc,
                numdelims: numdelims,
    ...
    var parseLinkDestination = function () {
        ...
        if (res === null) {
            ...
            return normalizeURI$1(
                unescapeString$1(res.substr(1, res.length - 2))
            );
        }
    };
            ...
            continue: function (parser, container) {
                ...
                if (container._isFenced) {
                    ...
                    if (match && match[0].length >= container._fenceLength) {
                        ...
                        while (
                            i > 0 &&
                            isSpaceOrTab(peek$1(ln, parser.offset))
                        ) {
                            parser.advanceOffset(1, true);
                            i--;
                        }
    ...
    var blockStarts = [
        ...
        function (parser) {
            if (
                !parser.indented &&
                peek$1(parser.currentLine, parser.nextNonspace) ===
                    C_GREATERTHAN
            ) {
                parser.advanceNextNonspace();
                parser.advanceOffset(1, false);
        ...
        function (parser) {
            if (
                !parser.indented &&
                reThematicBreak.test(
                    parser.currentLine.slice(parser.nextNonspace)
                )
            ) {
                parser.closeUnmatchedBlocks();
